...
function VideoPlayer(config) {
  ...
  return {
    ...
    setPlayready: function () {
      ...
      if (
        chosenDrm.licenseServer !== "" &&
        chosenDrm.licenseServer !== undefined
      ) {
    ...
    setWidevine: function () {
      ...
      var drmParam =
        "DEVICE_ID=" +
        deviceId +
        "|DEVICE_TYPE_ID=60|STREAM_ID=|IP_ADDR=|DRM_URL=" +
        chosenDrm.licenseServer +
        "|PORTAL=OEM|I_SEEK=|CUR_TIME=|USER_DATA=";
      if (chosenDrm.customData !== "" && chosenDrm.customData !== undefined) {
    ...
    getProperties: function () {
      var text =
        "AVAILABLE_BITRATE: " +
        webapis.avplay.getStreamingProperty("AVAILABLE_BITRATE") +
        "<br />";
      text +=
        "CURRENT_BANDWIDTH: " +
        webapis.avplay.getStreamingProperty("CURRENT_BANDWITH") +
        "<br />";
      text +=
        "DURATION: " +
        webapis.avplay.getStreamingProperty("DURATION") +
        "<br />";
      text +=
        "BUFFER_SIZE: " +
        webapis.avplay.getStreamingProperty("BUFFER_SIZE") +
        "<br />";
      text +=
        "START_FRAGMENT: " +
        webapis.avplay.getStreamingProperty("START_FRAGMENT") +
        "<br />";
      text +=
        "COOKIE: " + webapis.avplay.getStreamingProperty("COOKIE") + "<br />";
      text +=
        "CUSTOM_MESSAGE: " +
        webapis.avplay.getStreamingProperty("CUSTOM_MESSAGE");
      info.innerHTML = text;
    },
